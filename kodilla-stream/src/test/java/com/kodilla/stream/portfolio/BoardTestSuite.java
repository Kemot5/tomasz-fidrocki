...
public class BoardTestSuite {
    ...
    private Board prepareTestData() {
        ...
        //Tasks
        Task task1 = new Task("Microservice for talking temperature",
                "Wite and test the microservice taking\n" +
                        "the temerature from external service",
                user1,
                user2,
                LocalDate.now().minusDays(20),
                LocalDate.now().plusDays(30));
        Task task2 = new Task("HQLs for analysis",
                "Prepare some HQL quersies for analysis",
                user1,
                user2,
                LocalDate.now().minusDays(20),
        ...
        Task task5 = new Task("Optimize searching",
                "Archive date searching has to be optimized",
                user4,
                user2,
                LocalDate.now(),
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        List<TaskList> inProgressTask = new ArrayList<>();
        inProgressTask.add(new TaskList("In progress"));
        var days = project.getTaskLists().stream()
                .filter(inProgressTask::contains)
                .flatMap(d ->d.getTasks().stream())
                .map(e-> Period.between(e.getCreated(), LocalDate.now()))
                .mapToDouble(c ->c.getDays())
                .average();


                // nie wiem z kad te dni sie biorą( czemu tu sa 3 dni)???????????????????
